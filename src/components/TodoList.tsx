...
const TodoList = ({ onSignOut }: { onSignOut: () => void }) => {
  ...
  const filteredTodos = todos.filter((todo) => {
    ...
    const matchesFilter = filter === "all" ? true : filter === "open" ? !todo.completed : todo.completed;
      ...
      <main className="mx-auto w-full max-w-5xl space-y-8 px-6 py-8">
            ...
            <div className="flex items-center gap-2 rounded-xl bg-secondary/50 p-1">
              {(["all", "open", "completed"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold uppercase tracking-wider transition-all ${filter === f ? "bg-white text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"
                    }`}
                >
                  {f}
                </button>
        ...
        <section className="grid gap-6 md:grid-cols-3">
          ...
          <div className="md:col-span-2 space-y-6">
            <form onSubmit={handleAdd} className="premium-card rounded-2xl p-6 space-y-4 relative overflow-hidden">
